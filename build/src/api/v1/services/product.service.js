...
class ProductService {
    async create({ product_name, product_collection, product_vendor, product_description, tags, product_images, product_options = [{ name: 'Title', _position: 1 }], only_hide_from_list = false, not_allow_promotion = false, published_at, product_variants = [
        ...
        if (variantsResult.status !== 'fulfilled')
            throw new error_response_1.BadRequestError('Variant initialization failed');
        if (imagesResult.status !== 'fulfilled')
            throw new error_response_1.BadRequestError('Image initialization failed');
        ...
        await Promise.all([
            ...
            (0, collection_repo_1.addProductToCollection)(product_collection, newProduct._id),
        ]);
        return newProduct;
    }
