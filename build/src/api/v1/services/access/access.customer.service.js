...
class AccessCustomerService {
    ...
    async login({ email, password, }) {
        ...
        if (!foundCustomer)
            throw new error_response_1.AuthFailureError('Unauthorized');
        const match = await bcrypt_1.default.compare(password, foundCustomer?.password);
        if (!match)
            throw new error_response_1.AuthFailureError('Unauthorized');
    ...
    async getProfile(user) {
        const customer = await (0, customer_repo_1.findCustomerById)(user.userId, [
            'password',
            'state',
            'createdAt',
            'updatedAt',
