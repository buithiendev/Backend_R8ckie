...
class AccessCustomerService implements IAccessCustomerService {
    ...
    async login({
        ...
        if (!foundCustomer) throw new AuthFailureError('Unauthorized')
    ...
    async getProfile(user: PayloadToken): Promise<Customer | null> {
        const customer = await findCustomerById(user.userId, [
            'password',
            'state',
            'createdAt',
            'updatedAt',
