...
import { addProductToCollection } from '../../models/repositories/collection.repo'
import {
    insertManyImage,
...
class ProductService implements IProductService {
    async create({
        ...
        if (variantsResult.status !== 'fulfilled')
            throw new BadRequestError('Variant initialization failed')

        if (imagesResult.status !== 'fulfilled')
            throw new BadRequestError('Image initialization failed')
        ...
        await Promise.all([
            ...
            addProductToCollection(product_collection, newProduct._id),
        ])
        return newProduct
    }
